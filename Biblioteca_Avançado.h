...
No* Insere_Lista_Ordenada_Prioridade(No *lista,tarefa t,int ordem_prioridade)
{
    No* aux;
    No* aux_2;
    No* auxiliar_2;
    ...
    for(aux=lista ; aux != NULL ; aux = aux->prox)
    {
        if(aux->info.prioridade == ordem_prioridade)
        {
                if(novo->info.data_termino.ano <= aux->info.data_termino.ano)
                {
                    if(novo->info.data_termino.ano == aux->info.data_termino.ano)
...
No* Insere_Prioridade(No *lista,tarefa t)
{
    No *aux;
    No *anterior;
    No *novo;
    ...
    while (aux != NULL && novo->info.prioridade > aux->info.prioridade)
    {
        anterior = aux;
        aux = aux->prox;
    }

    if(anterior == NULL && novo->info.prioridade != aux->info.prioridade)
    {
        novo->prox = aux;
        lista = novo;
        return lista;
    ...
    if(novo->info.prioridade != aux->info.prioridade)
    {
        novo->prox = aux;
        anterior->prox = novo;
        return lista;
    }
    else // Analise de datas
    {
